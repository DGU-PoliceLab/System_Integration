...
def Emotion(data_pipe, event_pipe):
    ...
    while True:        
        ...
        if data:
            ...
            for i, track in enumerate(tracks):
                ...
                if face_detections is not None:
                    for i in range(face_detections.shape[0]):
                        ...
                        ##
                        logger.info(f"face개수 : {face_detections.shape[0]}, emotion Label: {emotion}, combine_data : {combine_data}")
            ...
            event_pipe.send({'action': emotion_index_list, 'id':tid_list, 'cctv_id':meta_data['cctv_id'], 
                            'current_datetime':meta_data['current_datetime'], 'location':meta_data['cctv_name'],
                            'combine_data': combine_data, 'db_insert_file_path':file_name_list}) #TODO action is not emotion
                    # event_count += 1 어떤 의미의 코드인지 모르겠음.                        
                    #TODO!!! combine_data is list not single data.
