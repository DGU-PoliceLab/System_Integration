...
def evaluate_frames(model, all_keypoint_batches):
    ...
    with torch.no_grad():  # Turn off gradients for prediction
        ...
        # putText를 위한 반으로 자르기
        all_scores1 = all_scores[:length // 2]
        all_scores2 = all_scores[length // 2:]
        # print(f"all_scores1 : {all_scores1}")
        # print(f"all_scores2 : {all_scores2}")
        # option 1 take max score
        max_score = round(torch.max(all_scores).item(),3)
        mean_score = round(torch.mean(all_scores).item(),3)
        draw_oringal_out_1 = f"Oriout_1 : {all_scores1}"
        draw_oringal_out_2 = f"Oriout_2 : {all_scores2}"
        draw_OriMax = f"OriMax : {max_score}"
        draw_OriMean = f"OriMean : {mean_score}"
        #print(draw_oringal_out)
