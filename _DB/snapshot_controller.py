...
def object_snapshot_control(input_queue):
    ...
    while True:
        if not input_queue.empty():
            # queue.get은 큐가 비어있으면 자동 대기함.
            # tracks, meta_data = queue.get(timeout=1)  # 큐에서 데이터를 꺼내고, 1초 동안 대기
            ...
            if first_run or (current_tid_count != previous_tid_count and current_tid_count <= MAX_PERSON_NUM):  # 첫 실행이거나 tid 개수가 다를 때만 실행하고 MAX_PERSON_NUM를 넘지 않도록
                ...
                for i, track in enumerate(tracks):
                    ...
                    people_thumbnail_location_link = str(tid) + "_" + str(current_datetime)
                    file_path = os.path.join(save_path, f"ID_{people_thumbnail_location_link}")
                    people_name_material = tracking_time.strftime("%y%m%d_%H%M%S")
