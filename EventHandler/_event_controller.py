...
from EventHandler._db_controller import insert_event, connect_db
from EventHandler._mq_controller import connect_mq
from queue import Queue

def collect_evnet(pipe, is_debug):
    logger = get_logger(name= '[EVENT]', console= True, file= True)
    
    if is_debug == False:
        import config
        conn = connect_db(config.db_config)
        mq_conn = connect_mq(config.mq_config)
