...
mq_logger = get_logger(name= '[MQ]', console= True, file= True)
db_logger = get_logger(name = '[DB]', console=False, file=False)
event_logger = get_logger(name= '[EVENT]', console= True, file= True)
    ...
    while True:
        ...
        if event is not None:
            ...
            try:
                ...
                db_logger.info(f"[EVENT DETECT] - cctv_id : {cctv_id}, event_type : {event_type}, event_location : {event_location}, track_id : {track_id}, event_date : {event_date}, event_time : {event_time}, event_clip_directory : {event_clip_directory}, event_start : {event_start}, event_end : {event_end}")
                ...
                if check(event_type, event_cur_time): # event insert delay code use here
                    ...
                    if people_table_id:
                        ...
                        db_logger.warn("'[INSERT_EVENT] : people_id' that satisfies the condition does not exist.")
                    ...
                    # conn.commit()
                    ####
                    LAST_EVENT_TIME[event] = event_cur_time
            except Exception as e:
                db_logger.warning(f"Error occurred in insert_event loop, error: {e}")
                pass
#endregion DB query
